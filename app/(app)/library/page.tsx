      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
        <Card className="overflow-hidden">
          ...
          <div className="space-y-3 p-4 sm:p-5">
            {content.length ? (
              content.map((item) => (
                <article key={item.id} className="rounded-md border border-navy-950/10 bg-white p-3">
            ...
            <div className="space-y-2 border-t border-navy-950/10 p-4 sm:p-5">
              {playlists.length ? (
                playlists.map((playlist) => (
                  <a
                    key={playlist.id}
                    className="focus-ring block rounded-md border border-navy-950/10 bg-white px-3 py-2 text-sm font-medium text-navy-950 underline"
                    href={playlist.url}
                    target="_blank"
                    rel="noreferrer"
                  >
