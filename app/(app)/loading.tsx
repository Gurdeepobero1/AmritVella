...
export default function AppLoading() {
  return (
    <div className="space-y-5" aria-live="polite" aria-busy="true">
      <section className="sunset-panel rounded-[16px] p-6 text-white sm:p-8">
        <div className="flex items-center gap-2 text-sm font-semibold text-white/70">
          <span className="h-2 w-2 rounded-full bg-saffron-500" />
          Opening today
        </div>
        <div className="mt-8 h-16 w-28 animate-pulse rounded-[12px] bg-white/15" />
        <div className="mt-4 h-4 w-44 animate-pulse rounded-full bg-white/15" />
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {loadingCards.map((card) => {
            const Icon = card.icon;
            return (
              <div key={card.label} className="rounded-[12px] bg-white/10 p-4">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[0, 1, 2, 3].map((item) => (
          <div key={item} className="rounded-[12px] border border-hairline bg-card p-4">
